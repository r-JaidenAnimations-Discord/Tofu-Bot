...
module.exports = async (client, message) => {
	...
	if (timestamps.has(message.author.id)) {
		...
		if (now < expirationTime) {
			try {
				await message.react('⏳');
				return message.reply(`It's cool you're trying to do stuff but could you chill a bit for ${humanReadableDuration(expirationTime - now)} before reusing \`${command.name}\`?`);
			} catch (e) {
